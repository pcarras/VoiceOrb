...
if (!gotTheLock) {
  ...
  ipcMain.handle("transcribe-audio", async (event, arrayBuffer) => {
    ...
    if (!apiKey) {
      return {
        error:
          "API Key Missing. Please set GROQ_API_KEY env var or create whisper4u.ini with {'api_keys': {'groq': 'YOUR_KEY'}}.",
      };
    }
  ...
  async function getGroqApiKey() {
    ...
    if (appConfig.api_keys && appConfig.api_keys.groq)
      return appConfig.api_keys.groq;
    return null;
  }
  ...
  function createWindow() {
    ...
    // --- Caps Lock Control ---
    function setCapsLock(turnOn) {
      // Use PowerShell to check and set CapsLock state
      // This is "fire and forget" to avoid blocking the main thread, though spawn is async.
      const script = `
        $w = New-Object -ComObject WScript.Shell;
        $active = [Console]::CapsLock;
        if (($active -and -not ${turnOn ? "$true" : "$false"}) -or (-not $active -and ${turnOn ? "$true" : "$false"})) {
            $w.SendKeys('{CAPSLOCK}');
        }
        `;

      const { spawn } = require("child_process");
      // bypass execution policy for this command
      const child = spawn("powershell", ["-NoProfile", "-Command", script]);

      child.on("error", (err) => {
        console.error("Failed to toggle CapsLock:", err);
      });
    }
    ...
    uIOhook.on("keyup", (e) => {
      ...
      const wasPartOfShortcut =
        appConfig.shortcut && appConfig.shortcut.includes(e.keycode);
